...
def read_last_email(account: str = DEFAULT_ACCOUNT) -> str:
    """קורא את המייל האחרון בלבד ומחזיר פירוט מלא בעברית."""
    try:
        from googleapiclient.discovery import build
        from core.google_auth import get_credentials
    except ImportError as exc:
    ...
    try:
        ...
        if not messages:
            return "לא נמצאו מיילים בתיבת הדואר."
        ...
        account_label = "האישי" if account == PERSONAL_ACCOUNT else "הארגוני"
        return f"המייל האחרון ב{account_label} הוא מ-{sender}, בנושא: {subject}."

    except Exception as exc:
        return f"לא הצלחתי לקרוא את המייל האחרון: {exc}"
